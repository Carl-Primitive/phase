...
Before Step 3, verify `IMPLEMENTATION_WORKTREE` — the checkout Tilt watches (normally the main checkout; never a `git worktree add`) — is clean at `START_SHA`. Every measurement and completion check after the checkpoint runs in that same checkout at `CANDIDATE_SHA` with a clean tree: `./scripts/verify-card.sh` for the gates and `./scripts/parse-diff-local.sh` for the parser projection. Do not create base, candidate, projection, or completion worktrees, and do not allocate a second `CARGO_TARGET_DIR` for anything: Tilt does not watch a second checkout (`tilt-wait.sh` answers exit 3 there), every additional target directory is a cold build of the engine crate, and the parse baseline for `BASE_SHA` is already published by CI and downloaded by `parse-diff-local.sh`. One checkout and one set of Tilt target roots serve the whole run.
...
In a chartered phase's impl loop, T4 runs two branches executed as decision revision: *(a) truncate* — the revision truncates phase k (its deferral list grows by the split-off remainder, attributed to new successor phases) and passes charter review; then one stabilization fix round, then the normal checkpoint → measure → completion → review sequence under phase mode with the truncated allowlist; zero findings → accepted (successors base on the truncated phase's accepted candidate); findings → resume normal fix rounds. Already-landed candidates remain the phase's rounds. *(b) restart* — restart phase k with the implementation checkout reset to its own `PHASE_BASE_SHA` under the revised charter; the abandoned candidates fork off the accepted chain, never appear in any chain-integrity interval, and are listed as abandoned in the Final Report.
...
**When both hold, switch modes** — measure them, do not judge them:

1. The last round's findings are all **spot** — a stale number, a stale coordinate, a claim contradicted by a neighbouring section, a missing restatement of a control the plan already specifies, a sentence never swept. None changes what the implementation does. A spot-only round is itself the measurement that the design has stopped moving: nothing in it names a step, sub-step, enum variant, or call site to add, remove, or replace. (An earlier form of this rule also required the design to be unchanged for two consecutive rounds before switching; on the Agent Frank Horrigan run that cost a full planner-plus-reviewer cycle for each of two single-nit rounds and bought nothing, because the whole-artifact re-review below is what catches an in-place rewrite that preserves every name.)
2. Each finding names a coordinate **and** its replacement text. If any finding requires *deciding* something, it is a design finding: stay in the loop.

**Do not add a third condition based on falling churn.** Round-over-round churn shrinks while a loop turns unproductive: smaller repairs to a growing record. It measures edit size, not convergence, and gating on it blocks the switch precisely when the switch is warranted.
...
This is not a licence for "two rounds and ship". The unbounded loop remains the default and the burden of proof is on leaving it: no measurement, no switch. Surgical mode is scoped to this Step 2 plan-review loop only — Step 6's implementation-review loop never uses it, because there the artifact is a committed candidate that only an executor may edit under the `SCOPE_PATHS` contract.
...
If the change touches the parser, find out whether it moves parser output: `./scripts/parse-diff-local.sh` diffs the published coverage baseline for the base commit (the same artifact PR CI diffs against) against the candidate's coverage data from Tilt's `card-data` resource — no second build, no second worktree. Report what changed.

### Step 5 — Verify the committed candidate

Run the checks against `CANDIDATE_SHA` with a clean tree — `git status --porcelain` empty at that commit — in the checkout Tilt watches. Never in a second worktree: Tilt does not watch it, `tilt-wait.sh` answers exit 3 from it, and every fresh worktree is a fresh cold build of the engine crate. `./scripts/verify-card.sh` prints `verify-card PASS head=<sha> tree=clean|dirty`; only a `tree=clean` line at `CANDIDATE_SHA` counts as evidence. A check that passes against uncommitted edits has told you nothing about what you are shipping. Run every gate the changed surface calls for: formatting for any implementation change, the Rust/engine/parser block for Rust paths, the frontend block for frontend paths, the parser gate for parser paths. Markdown-only policy changes need scope and diff checks; do not run Cargo or Tilt for them.
