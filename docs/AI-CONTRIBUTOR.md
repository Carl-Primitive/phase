...
`tilt up -- engine` is the **engine loop**: it builds, then keeps warm, exactly the three resources §6 is gated on — `card-data`, `clippy`, `test-engine` — and removes every client-side resource from the session. The first start is a cold build of the engine crate and takes a while; every later rebuild is incremental. Do not use plain `tilt up` (the client loop: wasm + frontend + lobby worker) or add `tauri`/`https` for card work; each adds full engine builds a card contribution never uses. After the first build only `build-native` reruns on every edit; the three gates are manual and `verify-card.sh` (via `tilt-wait.sh`) triggers them, so an edit never costs a clippy or card-data run you did not ask for. `gen-card-data.sh` no longer rewrites the tracked catalogs (`known-tokens.toml`, `oracle-subtypes.json`, `mtgjson-vintage`) locally — you build and test against the committed ones, as CI does; `PHASE_PROMOTE_CATALOGS=1` opts in.
